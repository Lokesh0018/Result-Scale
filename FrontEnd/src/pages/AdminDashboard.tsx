...
function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      try {
        ...
        const renderRes = await fetch(
          `${VITE_RENDER_API_URL}/admin/students`
        );
        ...
        showToast(
          err.message || "Failed to load Render students.",
          "error"
        );
        setRenderStudents([]);
      }
      ...
      try {
        ...
        const railwayRes = await fetch(
          `${VITE_RAILWAY_API_URL}/admin/students`
        );
        ...
        showToast(
          err.message || "Failed to load Railway students.",
          "error"
        );
        setRailwayStudents([]);
      }
    ...
    const fetchClients = async () => {
      try {
        const res = await fetch(`${VITE_RAILWAY_API_URL}/admin/dashboard`);
  ...
  const addOrUpdateClient = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (addOrUpdate) {
      try {
        const res = await fetch(`${VITE_RAILWAY_API_URL}/admin/clients`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        const newClient: Client = {
          ...
          status:
            new Date(data.client.portalExpiryDate).getTime() < Date.now()
              ? "expired"
              : "active",
  ...
  const deleteClient = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const userEmail =
      localStorage.getItem("userEmail") || "admin@resultscale.com";
    const userRole =
    ...
    try {
      ...
      const updatedClients = clients.filter(
        (client) => client.email !== deletedClient.email
      );
