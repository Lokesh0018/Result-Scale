...
function AdminDashboard() {
  ...
  const fetchQuotationRequests = () => {
    ...
    apiFetch('/admin/quotation-requests', { auth: true })
      .then((data) => {
        setQuotationRequests(data.requests || []);
      })
      .catch((err) => {
        if (err.message.includes('Unauthorized')) logout('/admin/login');
        else setQuotationsError(err.message);
      })
  ...
  const handleDeleteQuotation = async (id: string) => {
    try {
      await apiFetch(`/admin/quotation-requests/${id}`, { method: 'DELETE', auth: true });
  ...
  const fetchInquiries = () => {
    ...
    apiFetch('/admin/inquiries', { auth: true })
      .then((data) => {
        setInquiries(data.inquiries || []);
      })
      .catch((err) => {
        if (err.message.includes('Unauthorized')) logout('/admin/login');
        else setInquiryError(err.message);
    }).then((data) => {
      setInquiries(data.inquiries || []);
    }).catch((err) => {
  ...
  useEffect(() => {
    if (activeSection === "dashboard" || activeSection === "logs") {
      const fetchStudents = async () => {
        try {
          const data = await apiFetch('/admin/students', { auth: true });
