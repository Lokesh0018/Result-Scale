...
function AdminDashboard() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      let renderData = { students: [] };
      let railwayData = { students: [] };
      ...
      // Merge students by email or rollNo + institutionName to de-duplicate
      const merged = new Map<string, Student>();
      const allFetched = [...(renderData.students || []), ...(railwayData.students || [])];
      allFetched.forEach((student: Student) => {
        if (student && student.email) {
          merged.set(student.email, {
            ...merged.get(student.email),
            ...student
          });
        }
      });
  ...
  const mergeClientsByEmail = (
    renderClients: Client[],
    railwayClients: Client[]
  ) => {
    const merged = new Map<string, Client>();

    [...renderClients, ...railwayClients].forEach((client) => {
      const existing = merged.get(client.email);
      if (existing) {
        merged.set(client.email, {
          ...existing,
          ...client,
          students: (existing.students || 0) + (client.students || 0),
          source: existing.source && client.source && existing.source !== client.source 
            ? 'Render/Railway' 
            : (client.source || existing.source)
        });
      } else {
  ...
  useEffect(() => {
    ...
    const mapClients = (data: any[], source: string): Client[] =>
      (data || []).map((user: any, index: number) => {
        const portalExpiryDate = user.portalExpiryDate
          ? String(user.portalExpiryDate).split("T")[0]
          : "";
        const isExpired = user.portalExpiryDate
          ? new Date(user.portalExpiryDate).getTime() < Date.now()
          : false;
    ...
    const fetchClients = async () => {
      try {
        ...
        if (!railwayRes.ok) {
          throw new Error(await railwayRes.text());
        }
        ...
        const renderClientsList = mapClients(renderData.data, 'Render');
        const railwayClientsList = mapClients(railwayData.data, 'Railway');

        setRenderClients(renderClientsList);
        setRailwayClients(railwayClientsList);

        setRenderStudents(
          renderClientsList.reduce(
            (acc, client) => acc + (client.students || 0),
            0
          )
        );

        setRailwayStudents(
          railwayClientsList.reduce(
            (acc, client) => acc + (client.students || 0),
            0
          )
        );
    ...
    if (addOrUpdate) {
      try {
        const [renderRes, railwayRes] = await Promise.all([
          ...
          fetch(`${VITE_RAILWAY_API_URL}/admin/clients`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              "X-User-Email": userEmail,
        ...
        if (!railwayRes.ok) {
          throw new Error(railwayData.message);
        }

        const renderClient: Client = {
          id: clients.length + 1,
          _id: renderData.client._id,
          institutionName: renderData.client.institutionName,
          email: renderData.client.email,
          students: renderData.client.students || 0,
          status:
            new Date(renderData.client.portalExpiryDate).getTime() < Date.now()
              ? "expired"
              : "active",
          portalExpiryDate: renderData.client.portalExpiryDate
            .split("T")[0],
          institutionType: renderData.client.institutionType,
          logoUrl: renderData.client.logoUrl,
          isActive: renderData.client.isActive,
        };

        const railwayClient: Client = {
          id: clients.length + 1,
          _id: railwayData.client._id,
          institutionName: railwayData.client.institutionName,
          email: railwayData.client.email,
          students: railwayData.client.students || 0,
          status:
            new Date(railwayData.client.portalExpiryDate).getTime() < Date.now()
              ? "expired"
              : "active",
          portalExpiryDate: railwayData.client.portalExpiryDate
            .split("T")[0],
          institutionType: railwayData.client.institutionType,
          logoUrl: railwayData.client.logoUrl,
      ...
      try {
        const [renderRes, railwayRes] = await Promise.all([
          fetch(
            `${VITE_RENDER_API_URL}/admin/clients/${formData.oldEmail}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
          ...
          fetch(
            `${VITE_RAILWAY_API_URL}/admin/clients/${formData.oldEmail}`,
            {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
        ...
        // Use either response since both should contain the same data
        const clientData = railwayData.client;

        const updatedClient: Client = {
          id: existingId,
          _id: clientData._id || clientData.id,
          institutionName: clientData.institutionName,
          email: clientData.email,
          students: clientData.students || 0,
          status:
            new Date(clientData.portalExpiryDate).getTime() < Date.now()
              ? "expired"
              : "active",
          portalExpiryDate: clientData.portalExpiryDate
            .split("T")[0],
        ...
        setClients((prev) =>
          prev.map((client) =>
            client.email === formData.oldEmail
              ? updatedClient
              : client
          )
        );
  ...
  const deleteClient = async (
    ...
    try {
      ...
      if (!renderRes.ok) {
        throw new Error(
          renderData.message || "Failed to delete client"
        );
      }

      if (!railwayRes.ok) {
        throw new Error(
          railwayData.message || "Failed to delete client"
        );
      }
            ...
            <>
              <div className="stats-grid">
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-card-value">{renderStudents || 0}</div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-card-value">{railwayStudents || 0}</div>
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-card-value">{railwayStudents + renderStudents || 0}</div>
                                    ...
                                    onMouseEnter={(e) => {
                                      ...
                                      if (parentRect) {
                                        setTooltipData({
                                          ...
                                          value: `${(client.students || 0).toLocaleString()} Enrolled Students${client.source ? ` (${client.source})` : ''}`
                                        });
                                      }
                                    }}
      ...
      {selectedClient && (
        <div className="modal-overlay" onClick={() => setSelectedClient(null)}>
          <div className="modal" style={{ maxWidth: '650px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              {(() => {
                const clientStudents = students.filter((s: Student) => s.institutionName === selectedClient.institutionName);
                const avgSgpa = clientStudents.length > 0
                  ? clientStudents.reduce((acc: number, curr: Student) => acc + curr.sgpa, 0) / clientStudents.length
                  : 0;
                      ...
                      <div className="detail-table-container">
                        {clientStudents.length === 0 ? (
                          ...
                          <table className="detail-table">
                            ...
                            <tbody>
                              {clientStudents.map((student: Student) => (
